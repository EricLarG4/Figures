...
# This presentation is available</br>on [R.largy.fr](https://r.largy.fr) {background-color="#184669"}
...
# This presentation is not exhaustive {background-color="#c9521a"}

Experiment</br>
Ask questions</br>
Explore packages</br>
Read documentations</br>
...
#| label: libs
librarian::shelf(
  tidyverse, patchwork, data.table, ggtext, ggrepel, ggthemes, scales, gt, fakir, paletteer, ggsci, clauswilke/colorblindr, gapminder, gganimate, palmerpenguins, png, plotly, charlatan, hrbrmstr/ggalt, DT, reticulate
)
```
...
## The [data]{.fg style="--col: #cb962a"} to be visualized must have</br>1 column per variable and 1 row per observation {auto-animate="TRUE"}
...
#| output-location: column
#| classes: columns6040
a_null <- NULL # absence of value
a_missing <- NA # missing value
a_nan <- NaN # not a number
a_inf <- Inf # infinity
a_minus_inf <- -Inf # negative infinity
char_missing <- NA_character_ # missing character
real_missing <- NA_real_ # missing real number
int_missing <- NA_integer_ # missing integer
...
## A NULL value is not missing {auto-animate="TRUE" .code7}
...
## Factors are important for ordering elements {auto-animate="TRUE"}
...
factor_vec <- c(
  ...
  factor(
    new_data, 
    levels = c(levels(a_factor), "very high"))
)
factor_vec
```
:::

## Creating vectors {auto-animate="TRUE"}
...
## Subsetting vectors by position,</br>logical and negative indexing {auto-animate="TRUE"}
...
## Subsetting lists: `[ ]` vs. `[[ ]]` {auto-animate="TRUE" .code7}
...
#| output-location: column
#| classes: columns6040
# Select rows 1 to 3, columns "id" + "value"
a_df[1:3, c("id", "value")]
```
:::
...
## Pivoting data from wide to long format</br>with `data.table::melt()` {auto-animate="TRUE" .out7 .code6}
...
## Example for ggplots {auto-animate="TRUE" .code7}
...
#| code-line-numbers: "|5"
ggplot(
  diamonds[diamonds$carat < 1.5, ],
  aes(x = cut, y = carat, fill = price)
) +
  geom_tile()
```
...
ggplot(
  ...
  geom_raster() #high performance tiles
```
...
## Axes can be transformed with `trans` 🔬 {auto-animate="TRUE"}

```{r}
#| label: ggplot_aes_3_10b2
#| echo: true
#| eval: true
#| warning: false
#| message: false
...
ggplot(
  ...
  scale_x_continuous(
    limits = c(4, 6),
    trans = "log10" #show log10-transformed data
  ) +
  scale_y_continuous(
    limits = c(0, 2)
  )
...
## Same goes for discrete values with `scale_*_discrete` 🔬{auto-animate="TRUE" .code6}

```{r}
#| label: ggplot_aes_3_10b3
#| echo: true
#| eval: true
#| warning: false
#| message: false
...
# 4.2 Colour scales {background-color="#a5ca0f"}
...
#| fig-height: 6
#| fig-width: 6
#| code-line-numbers: "6,22|7,23-29"
ggplot(
  data = iris,
  aes(
    x = Sepal.Length,
...
## Make a grid of panels along 2 variable swith `facet_grid()` {auto-animate="TRUE"}
...
data.frame(x = 1:10, y = sample(1:10)) |>
  ...
  geom_bar(stat = "identity") +
  coord_trans(
    x = 'log10' #show non-transformed data
  )
```

## Polar coordinates, *&theta;* = *x* {auto-animate="TRUE"}
...
## Polar coordinates, *&theta;* = *y* {auto-animate="TRUE"}
...
## Map projections {auto-animate="TRUE" .code7}

```{r}
#| label: ggplot_aes_3_16b22
#| echo: true
#| eval: true
#| warning: false
#| message: false
...
## Example of an excellent map 

![[https://timogrossenbacher.ch/bivariate-maps-with-ggplot2-and-sf](https://timogrossenbacher.ch/bivariate-maps-with-ggplot2-and-sf)](resources/bm-thematic-bivariate-map-with-legend-1-2.png)

# 6. Themes {background-color="#ff69b4"}

## Preset themes {auto-animate="TRUE"}

```{r}
#| label: ggplot_aes_6_1a
#| echo: true
#| eval: true
#| warning: false
#| message: false
...
## Preset themes from `ggplot2` 🔬{auto-animate="TRUE"}

```{r}
#| label: ggplot_aes_6_1b
#| echo: true
#| eval: true
#| warning: false
#| message: false
...
## Preset themes from other libraries 🔬{auto-animate="TRUE" background-color="#002b36"}

```{r}
#| label: ggplot_aes_6_1e
#| echo: true
#| eval: true
#| warning: false
#| message: false
...
```
## Custom themes 🔬{auto-animate="TRUE" .code6}

```{r}
#| label: ggplot_aes_6_2a
#| echo: true
#| eval: true
#| warning: false
#| message: false
...
#| fig-height: 6
#| fig-width: 6
#| code-line-numbers: "13-30|15|16-20|21-23|24|25-28"
ggplot(
  data = diamonds,
  aes(x = carat, y = price, color = cut)
) +
...
## Custom themes recycling for consistency 🔬{auto-animate="TRUE" .code6}

```{r}
#| label: ggplot_aes_6_2b
#| echo: true
#| eval: true
#| warning: false
#| message: false
...
#| fig-height: 6
#| fig-width: 6
#| code-line-numbers: "1-18|32"
my_project_theme <- theme_minimal() +
  theme(
    panel.grid = element_blank(),
    axis.line = element_line(linewidth = 0.5),
    axis.ticks = element_line(linewidth = 0.5),
...
## Functionalize themes for fast tuning 🔬{auto-animate="TRUE" .code6}

```{r}
#| label: ggplot_aes_6_2c
#| echo: true
#| eval: true
#| warning: false
#| message: false
...
#| fig-height: 6
#| fig-width: 6
#| code-line-numbers: "1|5-11,13-15,17-18|12|34"
my_project_theme <- function(scale = 1, lgd = 'bottom') {
  theme_minimal() +
    theme(
      panel.grid = element_blank(),
      ...
      axis.title.x = element_text(size = 16 * scale),
      axis.title.y = element_text(size = 16 * scale),
      legend.title = element_text(size = 14 * scale, face = 'bold'),
      legend.text = element_text(size = 12 * scale),
      legend.position = lgd,
      strip.text = element_text(size = 12 * scale, face = 'bold'),
      title = element_text(face = 'bold', size = 18 * scale),
      plot.tag = element_text(face = 'bold', size = 18 * scale),
      plot.background = element_rect(color = NA, linewidth = NULL),
      plot.title = element_text(face = 'bold', size = 18 * scale),
    plot.caption = element_text(face = 'italic', size = 12 * scale)
    )
}
...
ggplot(
  ...
  my_project_theme(1.2, 'top')
```

# 7. Interactivity, combining and exporting {background-color="#184669"}

## Combine figures with `patchwork` 🔬{auto-animate="TRUE"}

```{r}
#| label: ggplot_aes_7_1
#| echo: true
#| eval: true
#| warning: false
#| message: false
...
## Increase layout control with `plot_layout` 🔬{auto-animate="TRUE"}

```{r}
#| label: ggplot_aes_7_3
#| echo: true
#| eval: true
#| warning: false
#| message: false
...
#| fig-height: 10
#| fig-width: 10
#| code-line-numbers: "1-11|1,10|11"
...
## Apply a layout with `plot_layout` 🔬{auto-animate="TRUE"}

```{r}
#| label: ggplot_aes_7_4
#| echo: true
#| eval: true
#| warning: false
#| message: false
...
## Apply themes and annotations globally with `&` 🔬{auto-animate="TRUE" .code7}

```{r}
#| label: ggplot_aes_7_5
#| echo: true
#| eval: true
#| warning: false
#| message: false
...
#| fig-height: 6
#| fig-width: 6
#| code-line-numbers: "11-13|14-19"

patch_iris <- patchwork::wrap_plots(
  plot_iris(
    "Sepal.Length",
     "Petal.Length"
     ),
  ...
  patchwork::plot_layout(ncol = 1) &
  my_project_theme(0.8) &
  theme(plot.tag = element_text(color = 'coral')) &
  patchwork::plot_annotation(
    tag_levels = "A",
    title = "Example of patchwork",
    subtitle = "Global annotations and themes with &",
    caption = "Data from ggplot2::iris"
  ) 
patch_iris

```

## Exporting plots with `ggsave` {auto-animate="TRUE"}
...
ggsave(
  filename = "resources/iris_patch.png",
  plot = patch_iris,
  width = 8,
  height = 8,
  units = "cm",
...
:::
::: {.column width="50%"}
![](resources/iris_patch.png)
:::
::::

::: footnote
[link to png file](https://github.com/EricLarG4/Figures/blob/28d00e6b8035acee4bf5403e70fe880db84227a8/resources/iris_patch.png)
:::


## Multiple formats are available {auto-animate="TRUE"}
...
ggsave(
  filename = "resources/iris_patch.svg",
  plot = patch_iris,
  width = 8,
  height = 8,
  units = "cm",
...
:::
::: {.column width="50%"}
![](resources/iris_patch.svg)
:::
::::

::: footnote
[link to svg file](https://github.com/EricLarG4/Figures/blob/28d00e6b8035acee4bf5403e70fe880db84227a8/resources/iris_patch.svg)
:::

## Use `ggplotly` to make plots interactive in 1 line 🔬{auto-animate="TRUE" .code6}
...
## Visuals are an increasingly important</br>form of science communication

Many scientists not well trained in design *principles* for [**effective**]{.fg style="--col: #fcbac5"} messaging

Visuals can be improved by taking some simple steps</br>before, during, and after their creation
...
[**Prioritize the information you want to share**]{.fg style="--col: #fcbac5"} and design it.

-   What [**core information**]{.fg style="--col: #fcbac5"} needs to be conveyed?
...
-   Examples
    ...
    -   MATLAB
    -   Python
    -   [**R**]{.fg style="--col: #fcbac5"}

# Principle 3:</br>Use an Effective Geometry</br>and Show Data {background-color="#ffdae0"}

## Less is more

-   Remove as much non-data-ink as possible
...
-   Do not include too much information
    ...
    -   Different message? Different figure!
...
## Compositions with pie charts do not work
...
## Stacked and clustered bars are preferable
...
## Normalized bars better show composition
...
## Make your own figure 🧪 {auto-animate=TRUE}

- Try to reproduce, improve or create your own figure with the WebMD dataset
  - Idea: show the spread of *Stuffy Nose* `Condition` across `Age` and `Gender`

- Data
  - [Source](https://www.kaggle.com/datasets/rohanharode07/webmd-drug-reviews-dataset)
  - [Pre-cleaned dataset](https://raw.githubusercontent.com/EricLarG4/Figures/refs/heads/main/data/webmd_clean.csv)

- Remember: you can read the file directly from the URL with 
  - `read.csv()`
  - `readr::read_csv()`
  - `data.table::fread()`

## Make your own figure 🧪🔬 {auto-animate=TRUE .code5}
...
#| fig-height: 6
#| fig-width: 6
#| code-line-numbers: "|5-9|11-18|22|23-25|26-32|33-34"
library(data.table)
fread(
  "https://raw.githubusercontent.com/EricLarG4/Figures/refs/heads/main/data/webmd_clean.csv"
) |>
  _[
    Condition == "Stuffy Nose" & Age != "",
    .(n = .N),
    by = c("Age", "Gender")
  ] |>
  ggplot(aes(
...
## Distributions are an underused category
...
fwrite(df.antibes, 'data/antibes_clean.csv')
...
## Bar plots are not appropriate for showing means</br>no distribution information and suggesest start at 0! :vomiting_face:
...
## Introducing data spread info with error bars</br>but what do they represent here?
...
## Scatter is less misleading</br>but the distribution information is still lacking
...
## Mean ± sd are identical for these (fake) distributions
...
## Density can be shown adding transparent data points
...
## Density can be shown adding jittered data points
...
## Combining jitter and transparency can be effective
...
## Violin plots are effective to compare distributions
...
## Most presentations of x- and y-coordinate data</br>are relationships
...
## Scatter plot starts with points
...
## Scatter plot can include lines
...
## Scatter plot can include models
...
- Try to reproduce, improve or create your own figure with the Antibes dataset
- Data
  - [Up-to-data source](https://www.data.gouv.fr/datasets/temperature-de-leau-de-mer-a-antibes-juan-les-pins)
  - [Dataset used here](https://raw.githubusercontent.com/EricLarG4/Figures/refs/heads/main/data/antibes-eau-temperature-est-cap.csv)
  - [Pre-cleaned dataset](https://raw.githubusercontent.com/EricLarG4/Figures/refs/heads/main/data/antibes_clean.csv)
- Remember: you can read the file directly from the URL with 
  - `read.csv()`
  - `readr::read_csv()`
  - `data.table::fread()`
...
## Bubble plot adds a symbol scale aesthetics</br>to map an additional variable
...
## Lollipop plots are less misleading</br>to show group means than bar plots
...
## Dumbbell plot allows multiple comparisons</br>between two points.
...
[**Colors should be strategically used to amplify your visual message**]{.fg style="--col: #fcbac5"}
...
## Sequential coloring can be light to dark</br>with [one hue]{style="background: linear-gradient(-90deg, #fde0dd, #fa9fb5, #f768a1, #c51b8a, #7a0177); -webkit-background-clip: text; background-clip: text; -webkit-text-fill-color: transparent; color: transparent;"}
...
## Sequential coloring can be light to dark</br>with [several hues]{style="background: linear-gradient(90deg, #440154, #3b528b, #21908d, #5dc863, #fde725); -webkit-background-clip: text; background-clip: text; -webkit-text-fill-color: transparent; color: transparent;"}
...
## Diverging coloring uses</br>[two sequential schemes and one neutral color]{style="background: linear-gradient(90deg, #d73027, #ffffbf, #4575b4); -webkit-background-clip: text; background-clip: text; -webkit-text-fill-color: transparent; color: transparent;"}
...
## [Highlighting]{.fg style="--col: #eb5181"} discrete values can be very effective</br>to convey a message
...
## Data highlighting can be reinfored</br> using <span style="text-decoration-line: underline; text-decoration-style: dashed; text-decoration-color: #eb5181; text-underline-offset: 0.1em;">*few* other elements</span>
...
- Try to reproduce, improve or create your own figure with the Antibes dataset
  - Idea: highlight outlying water temperatures in any given year in Antibes
- Data
  - [Up-to-data source](https://www.data.gouv.fr/fr/datasets/temperature-de-leau-de-mer-a-antibes-juan-les-pins)
  - [Dataset used here](https://raw.githubusercontent.com/EricLarG4/Figures/refs/heads/main/data/antibes-eau-temperature-est-cap.csv)
  - [Pre-cleaned dataset](https://raw.githubusercontent.com/EricLarG4/Figures/refs/heads/main/data/antibes_clean.csv)

## Make your own figure 🧪🔬 {.code5}
...
#| fig-width: 6
#| fig-height: 6
#| code-line-numbers: "|5|7-16|18-25|26-28|29-38|39-51|52-55|56-67|68-76"
library(data.table)
library(ggplot2)
library(ggtext)

thresholds <- c('cold' = 13, 'mild' = (13 + 27) / 2, 'hot' = 27)

dt_antibes <- fread(
  "https://raw.githubusercontent.com/EricLarG4/Figures/refs/heads/main/data/antibes_clean.csv"
) |>
  _[,
    category := fcase(
      temp < min(thresholds) , "cold" ,
      temp > max(thresholds) , "hot"  ,
      default = "mild"
    )
  ]
...
dt_antibes |>
  ...
  scale_colour_manual(
    ...
    values = c("cold" = "steelblue", "mild" = "grey30", "hot" = "tomato")
  ) +
  scale_y_continuous(
    breaks = thresholds,
    labels = paste0(
      ...
      c(
        ...
        '<span style="color:grey30">20</span>',
        '<span style="color:tomato">27</span>'
      ),
      '</b>'
    )
  ) +
...
-   Combine colors with symbols, line type and other elements</br>to convey the same information
...
## Use effective redundancy</br>to increase understanding and accessibility
...
## There are simple ways to include uncertainty 
...
## Integrating uncertainty with other elements</br>can be effective
...
## Many common metrics</br>but each represents a different measure
...
## When several aesthetics are redundant...
...
## ...one can be remapped to an additional variable
...
## Convey a message by faceting 🧪

- Goal: Highlight *where* (in which station) and *when* the pH deviated from 7
- Data
  - [Surface water quality in Loire-Atlantique](https://www.data.gouv.fr/fr/datasets/suivi-qualite-des-eaux-superficielles-en-loire-atlantique-stations-du-reseau-departemental/)
  - [Dataset used here](https://raw.githubusercontent.com/EricLarG4/Figures/refs/heads/main/data/qualite-eaux.csv)
  - [Pre-cleaned dataset](https://raw.githubusercontent.com/EricLarG4/Figures/refs/heads/main/data/qualite-eaux-clean.csv)
...
fwrite(dh, 'data/qualite-eaux-clean.csv')
...
## Highlighting, axis simplification, scaling, grid elements{.code5}
...
#| fig-height: 6
#| fig-width: 6
#| code-line-numbers: "|1-9|10|11-16|17|18|19-24|25-29|30-35"
fread("https://raw.githubusercontent.com/EricLarG4/Figures/refs/heads/main/data/qualite-eaux-clean.csv") |> 
  _[parameter == 'pH' & value > 2] |>
  _[,
  status := fcase(
    value > 9 , 'pH > 9'     ,
    value < 6 , 'pH < 6'     ,
  ...
  geom_line(color = 'grey', alpha = 0.25) +
  scale_color_manual(values = c('grey', 'tomato', 'steelblue')) +
  facet_wrap(~code_station, ncol = 8) +
  labs(
    y = 'pH',
    x = 'Date'
  ) +
...
## Showing the data to start with
...
## Adding transparency + trends goes nowhere
...
## Faceting per city group + trends is useful
...
## Recoloring along the variable of interest
...
## Statistical comparison to find significant deviations
...
## Simplification to get a clear message
...
## Simplification to get a clear message {.code5}
...
#| fig-height: 6
#| fig-width: 6
#| code-line-numbers: "|1-26|27-29|30-47|48-58|59|60|61|62-66|67-72"
fread(
  "https://raw.githubusercontent.com/EricLarG4/Figures/refs/heads/main/data/qualite-eaux-clean.csv"
) |>
  _[parameter == 'pH' & city_group != ''] |>
  _[,
    year := as.integer(format(date, "%Y"))
  ] |>
  _[,
...
# Principle 7:</br>Data and models</br>are different things {background-color="#ffdae0"}
...
## Model the pH vs. water T° relationship per city group 🧪 {.code5}
...
#| fig-height: 6
#| fig-width: 6
#| code-line-numbers: "|1-17|18-26|"
dt <- fread(
  "https://raw.githubusercontent.com/EricLarG4/Figures/refs/heads/main/data/qualite-eaux-clean.csv"
) |>
  _[city_group != '', .(parameter, value, date, city_group)]

dt_water <- dt[
  parameter == "Température de l'Eau",
  .(temp = value, date, city_group)
]

dt_pH <- dt[
  parameter == "pH",
  .(pH = value, date, city_group)
]

dt_water[dt_pH, on = .(date, city_group), allow.cartesian = TRUE] |>
  _[temp < 40, .(temp, pH, date, city_group)] |>
  ggplot(aes(temp, pH)) +
  geom_point(alpha = 0.05) +
  geom_smooth(
    method = 'lm',
...
## Legends and axis should be clear, complete and simple
